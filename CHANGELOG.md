...
- **L7014 is one row in /explore, and one layer on the map.** The pre-tiled mosaic and the nine hand-placed city sheets are complementary, not alternative: the mosaic's hole over Saigon *is* what the city sheets cover. `buildSeriesRows` folds a raster archive together with the database series it declares itself `halfOf`, so one tap adds the survey and the row reads *461 of 627 sheets*.
- **A survey is one stack row too.** It was two — mosaic and city sheets, an opacity slider, an eye and an × each, two of the ten stack slots — so editing one survey meant editing two layers in step, and the halves could be dragged apart. `RasterRef` and `SheetsRef` are now one `SeriesRef` carrying `parts`, and `LayerRenderer` gives each row two z-slots so the warped sheets still draw above the mosaic's pixels. A stack saved before this reads back through `readOverlayRef` (each half as a one-part row) and `foldLegacyOverlays` (the pair merged in the higher of their two positions, keeping its opacity); a survey saved as its sheets half alone comes back with the mosaic under it.
- The `?map=` written when a layer is removed is the topmost **sheet**, not the topmost row. A series left on top has no catalogue row, so its synthetic id reached the URL as a share link that opens an empty page.
