...
const svgText = (x, y, text, css = 'label') =>
  text
    .split('\\n')
    .map((line, i) => `<text x="${x}" y="${y + i * 23}" class="${css}">${esc(line)}</text>`)
    .join('');
const box = (x, y, width, height, text, tone = 'blue') =>
  `<rect x="${x}" y="${y}" width="${width}" height="${height}" rx="12" class="box ${tone}"/>${svgText(x + width / 2, y + height / 2 - (text.split('\\n').length - 1) * 11 + 7, text)}`;
const arrow = (x1, y1, x2, y2, dashed = false) =>
  `<path d="M ${x1} ${y1} L ${x2} ${y2}" class="arrow${dashed ? ' dashed' : ''}" marker-end="url(#arrow)"/>`;
...
function renderDiagrams() {
  ...
  const text = (x, y, value, css = 'small') =>
    `<text x="${x}" y="${y}" class="${css}" style="text-anchor:start">${esc(value)}</text>`;
  const rect = (x, y, w, h, fill, stroke = fill, dash = '') =>
    `<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="${fill}" stroke="${stroke}" stroke-width="2" stroke-dasharray="${dash}"/>`;
  const panel = (x, title) =>
    rect(x, 65, 450, 265, '#f8fafc', '#e2e8f0') + text(x + 20, 98, title, 'label');
  writeFigure(
    'figure-1',
    svg(
      1000,
      ...
      svgText(500, 30, 'A shared error can leave the check unchanged', 'label') +
        panel(30, 'A · Before a shared shift') +
        panel(520, 'B · After a shared shift') +
        text(55, 150, 'Control') +
        text(55, 215, 'Graticule') +
        text(545, 150, 'Control') +
        text(545, 215, 'Graticule') +
        ...
        arrow(740, 145, 870, 145) +
        arrow(740, 210, 870, 210) +
        `<circle cx="870" cy="145" r="9" fill="#eb6834"/><circle cx="870" cy="210" r="9" fill="#eb6834"/>` +
        text(55, 290, 'Their difference is zero.') +
        text(545, 290, 'Both move by the same amount: still zero.') +
        svgText(
          500,
          367,
          '(control + shift) − (graticule + shift) = control − graticule',
          'label'
        ) +
        svgText(
          500,
          403,
          'Measured in L7014: all 269 displaced sheets passed; median displacement 455 m.',
          'small'
        )
    )
  );
    ...
    svg(
      ...
      (() => {
        ...
        cases.forEach(([title, da, db, note], i) => {
          ...
          out +=
            rect(x + 30, 130, 110, 125, 'none', '#94a3b8', '5 5') +
            rect(x + 140, 130, 110, 125, 'none', '#94a3b8', '5 5');
          out += rect(
            x + 30 + da,
            145,
            110,
        ...
        return (
          ...
          svgText(
            500,
            350,
            'Dashed outlines: unshifted cells. Coloured blocks: placed sheets. Schematic, not to scale.',
            'small'
          ) +
          svgText(
            500,
            395,
            'A common shift needs an outside reference. An enforced seam cannot serve as this test.',
            'label'
          )
        );
      })()
  ...
  writeFigure(
    ...
    svg(
      ...
      svgText(500, 30, 'Which population answers which question?', 'label') +
        text(35, 85, 'L7014 · controlled reproduction', 'label') +
        text(560, 85, 'Indochine · distinct audits', 'label') +
        text(35, 132, '627 indexed cells') +
        arrow(215, 125, 255, 125) +
        text(270, 132, '510 GeoPDFs held') +
        text(35, 183, '437 warped in faulty pass') +
        text(35, 219, '269 displaced + 168 placed') +
        text(35, 270, '436 warped in corrected pass') +
        text(35, 306, '434 on cell + 2 over 150 m off') +
        text(560, 132, '79 indexed cells / 75 held') +
        text(560, 183, '62 pipeline-georeferenced sheets') +
        text(560, 234, '58 sheets read for the lattice check') +
        text(560, 285, '83 image sources in the tile audit') +
        rect(35, 335, 930, 48, '#f1f5f9') +
        text(
          50,
          364,
          '514 = recorded processing total (452 + 62); 452 is not a verified live manifest count.'
        )
    )
  );
    ...
    svg(
      ...
      svgText(550, 34, 'What one sheet supplies, and what the check compares', 'label') +
        ...
        box(
          ...
          360,
          140,
          'graticule_error compares A against\\ncontrol points read under B.\\nA shared datum error is invisible.',
          'rose'
        ) +
        svgText(
          550,
          556,
          'The two operands are not independent: the printed graticule and the control points are',
          'small'
        ) +
        svgText(
          550,
          581,
          'expressed in the same declared datum, so a datum error cancels out of their difference.',
          'small'
        )
    )
  );
    ...
    svg(
      ...
      (() => {
        ...
        let out =
          ...
          `<rect x="322" y="52" width="16" height="16" rx="3" fill="#eb6834"/>` +
          svgText(392, 61, 'faulty', 'small') +
          `<rect x="462" y="52" width="16" height="16" rx="3" fill="#2a78d6"/>` +
          svgText(534, 61, 'corrected', 'small') +
        ...
        return (
          ...
          svgText(
            500,
            398,
            'Seams over 100 m: 189 / 717 before; 0 / 715 after. Separate builds, not matched pairs.',
            'small'
          )
        );
      })()
    ...
    svg(
      ...
      (() => {
        ...
        const lon0 = 101.99,
          lat0 = 8.24;
        const latPx = 34.0,
          lonPx = latPx * Math.cos((15.9 * Math.PI) / 180);
        ...
        let out = svgText(
          500,
          34,
          'Where the datum fault fell: 627 cells of AMS Series L7014',
          'label'
        );
        ...
        for (const lat of [10, 15, 20]) {
          out +=
            `<line x1="74" y1="${my(lat).toFixed(1)}" x2="84" y2="${my(lat).toFixed(1)}" stroke="#86847b" stroke-width="1.5"/>` +
            svgText(44, my(lat), `${lat}°N`, 'small');
        }
        // the correctly-placed band, 14–17°N
        out +=
          `<path d="M 352 ${my(17).toFixed(1)} L 366 ${my(17).toFixed(1)} L 366 ${my(14).toFixed(1)} L 352 ${my(14).toFixed(1)}" fill="none" stroke="#0b0b0b" stroke-width="1.8"/>` +
        ...
        legend.forEach(([c, head, sub], i) => {
          const y = 120 + i * 78;
          out +=
            `<rect x="620" y="${y}" width="26" height="26" rx="3" fill="${fill[c]}" stroke="${edge[c]}" stroke-width="1.5"/>` +
        ...
        out +=
          `<line x1="620" y1="448" x2="960" y2="448" stroke="#b3b1a7" stroke-width="1.5"/>` +
    ...
    svg(
      ...
      (() => {
        let out = svgText(
          500,
          30,
          'The correction changes how many sheets reach their cell',
          'label'
        );
        for (const [y, name, total, placed, off] of [
          [115, 'Faulty', 437, 161, 276],
          [240, 'Corrected', 436, 434, 2],
        ]) {
          const w = (700 * placed) / 437,
            bad = (700 * off) / 437;
        ...
        return (
          ...
          svgText(
            500,
            355,
            'Separate dry-run builds. One fewer sheet warps after correction. Public mosaic not rebuilt.',
            'small'
          )
        );
      })()
  ...
  writeFigure(
    ...
    svg(
      ...
      svgText(500, 30, 'A full-resolution success misses the failing zoom level', 'label') +
        text(35, 95, 'Full resolution', 'label') +
        text(35, 195, 'One level below overview', 'label') +
        rect(350, 70, 600, 40, '#deebfc') +
        text(350, 140, '0% of tiles fail') +
        rect(350, 170, 600, 40, '#deebfc') +
        rect(350, 170, 600 * 0.586, 40, '#eb6834') +
        text(350, 240, '58.6% of tiles fail across the 83-image-source survey') +
        svgText(
          500,
          295,
          'Requested width-only size → stored explicit width,height key: audit the actual tile requests.',
          'small'
        )
    )
  );
  writeFigure(
    ...
    svg(
      ...
      svgText(500, 30, 'Keep the evidence attached to the build it describes', 'label') +
        text(35, 95, 'PUBLIC ARCHIVE', 'label') +
        text(365, 95, 'LOCAL REPRODUCTION', 'label') +
        text(695, 95, 'PLANNED DEPOSIT', 'label') +
        text(35, 145, 'l7014-20260913') +
        text(35, 180, 'Pixels served; manifest 404') +
        text(35, 215, '452 is an unverified constant') +
        text(365, 145, 'Faulty and corrected runs') +
        text(365, 180, 'Logs, seams and cell checks') +
        text(365, 215, 'Evidence for the correction') +
        text(695, 145, 'Versioned derived data + code') +
        text(695, 180, 'Zenodo DOI not yet minted') +
        text(695, 215, 'Scans remain at institutions') +
        svgText(
          500,
          285,
          'A successful local correction does not establish what a public viewer receives.',
          'label'
        )
    )
  );
...
function inline(sourceText) {
  ...
  let value = sourceText
    .replace(/`([^`]+)`/g, (_, code) =>
      keep(code === '∅' ? '\\ensuremath{\\varnothing}' : `\\texttt{\\detokenize{${code}}}`)
    )
...
function table(rows) {
  ...
  const spec = landscape
    ? left(0.22) +
      ...
      Array.from(
        { length: columns - 2 },
        () => `>{\\centering\\arraybackslash}p{${(0.69 / (columns - 2)).toFixed(3)}${basis}}`
      ).join('')
