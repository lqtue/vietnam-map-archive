...
const cols = 'id,slug,name,original_title,year,year_label,creator,dc_publisher,dc_description,dc_subject,dc_coverage,source_type,source_url,holding_institution,collection,iiif_image,status';
...
const { data: l7014sample } = await db.from('maps').select('slug,source_type,iiif_image').eq('slug', 'phat-diem-w').single();
console.log('---- phat-diem-w (sample iiif-warning row) ----');
console.log(JSON.stringify(l7014sample, null, 2));
const { data: srcs } = await db.from('map_iiif_sources').select('*').eq('map_id', (await db.from('maps').select('id').eq('slug','phat-diem-w').single()).data.id);
